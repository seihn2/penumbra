...
function getModel(_settings: AppSettings) {
  const fallbackModel = recommendedModelFor(_settings.apiBaseURL)?.id || 'gpt-5.6-luna'
...
const ASSIST_SYSTEM_PROMPT = `你是面试实时助手。面试官刚提出一个问题，请给候选人能"当场照着念"的回答要点。
...
输出要求：
- 用中文，3-6 个要点，每点一行，以"- "开头，只输出要点本身，不要前言、标题或总结。
- 第一条放最关键的，让候选人一开口就切中要害。
- 口语化、可直接念出口；用短句，避免书面长句和术语堆砌，必要术语点到为止。`
...
export function streamInterviewAssist(
  ...
  const openai = createOpenAI({ baseURL: settings.apiBaseURL, apiKey: settings.apiKey })
  ...
  const system = memory
    ? `${ASSIST_SYSTEM_PROMPT}${langLine}\n\n候选人背景（可结合其经历举例，但不要编造）：\n${memory}`
    : `${ASSIST_SYSTEM_PROMPT}${langLine}`
  const { textStream } = streamText({
    model: openai.chat(getModel(settings)),
    system,
...
/** Fetch the platform's available models from the OpenAI-compatible
   `/models` endpoint, so users can discover what their provider supports
   instead of typing model names by hand. */
